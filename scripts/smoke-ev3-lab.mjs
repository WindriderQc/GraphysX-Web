...
export async function runEv3Lab({ part = "all" } = {}) {
  if (!["all", "scene", "drive", "program"].includes(part)) throw new Error(`Unknown EV3 scenario: ${part}`);
  ...
  try {
    ...
    if (part !== "scene") {
      ...
      if (part === "all" || part === "program") {
        ...
        // The combined journey arrives from Drive; reproduce that handoff in an isolated run.
        if (part === "program") await page.locator("[data-ev3-mode='drive']").click();
  ...
  console.log(`\nEV3 Robotics Mission Lab ${part} smoke passed.`);
}
