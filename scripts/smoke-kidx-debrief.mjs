...
export async function runKidxDebrief({ part = "all" } = {}) {
  assert.ok(["all", "review", "turns", "outcomes"].includes(part), `Unknown debrief scenario: ${part}`);
  ...
  try {
    ...
    if (part === "all" || part === "turns") {
      if (part === "turns") {
        // The review journey leaves the previous attempt saved and adds three blocks.
    ...
    console.log(`KidX debrief ${part} smoke passed; no browser errors.`);
  } finally {
