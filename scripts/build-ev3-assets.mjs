...
model.traverse((mesh) => {
  ...
  geometry.applyMatrix4(mesh.matrixWorld); geometry.deleteAttribute("uv");
...
const provenance = { source: "LDraw official parts library; original KidX chassis layout and motor cables",
  ...
  parts: placements.map(({ line, ...placement }) => placement), files: credits };
await writeFile(new URL("ev3-driving-base.json", directory), JSON.stringify({
  catalog: { label: "EV3 Technic Driving Base", category: "vehicle" }, provenance, meshes,
}));
