...
no longer competes for the front door. The verify gate (typecheck + build + 28 browser/node
...
lighting is still synthetic-only. Ordered by impact-per-effort:
...
4. **Scene-authored IBL look controls, then HDRI** — cube skies already drive PMREM image-based
   lighting, and sky PMREM targets are now cached and ownership-safe. First expose environment
   intensity/rotation, background intensity/blur, and
   sky-vs-studio lighting using the existing `RoomEnvironment`. Then add one or two licensed 1K
   HDRIs and a focused recovered-material migration from Phong to Physical; HDR alone cannot
   improve headline archive meshes whose materials ignore `scene.environment`.
