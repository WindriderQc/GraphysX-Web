...
3. ~~World 1~~, ~~Skybox Spiral~~, and the ~~adapted Great Slide gravity run~~ are reachable ports.
   Map 1's exact 699-vertex/1456-triangle mesh is now catalogued for scene-native use, but **its
   game composition, an archive-faithful Great Slide rules reconstruction, and Level1 2011 remain**,
   each needing provenance notes and a gameplay smoke. Do not relabel the adapted run as full
   fidelity without new source evidence for the original rules.
