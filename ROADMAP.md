...
4. ~~**Scene-authored IBL look controls**~~ **Done** — Automatic/Sky/Studio source, reflection
   intensity, aligned yaw, backdrop intensity/blur, Natural/Soft/Hero editor looks, cached-PMREM
   reuse, and renderer/persistence coverage are scene vocabulary. **Next: HDRI + materials.** Add
   one or two licensed 1K HDRIs and a focused recovered-material migration from Phong to Physical;
   HDR alone cannot improve headline archive meshes whose materials ignore `scene.environment`.
