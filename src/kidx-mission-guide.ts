...
export const KIDX_MISSION_LANDMARKS: Record<string, [string, string]> = {
  ...
  "cargo-push": ["C’est la caisse jaune qui doit atteindre le bleu. Garde-la devant le robot pour la pousser.", "Dans Laboratoire +, B et C sont les deux moteurs. Compare leur puissance et la durée du déplacement."],
...
export function kidxMissionGuideStep(mission: Pick<KidxMission, "id" | "suggested" | "code">, state: GuideSnapshot): GuideStep {
  if (state.phase === "complete") return { key: "complete", step: 3, title: "Défi réussi !", text: state.attempt === "drive"
    ...
    ? "Tu peux enregistrer ton programme dans Laboratoire +, avec Enregistrer, ou revenir à l’Atelier pour choisir un autre défi."
  ...
  if (state.running && state.paused) return { key: "paused", step: 3, title: "Programme en pause", text: "Dans Laboratoire +, Reprendre continue le programme. Un bloc permet de regarder chaque action séparément.", target: state.labOpen ? "[data-code-resume]" : "[data-kidx-lab]" };
  if (state.running) return { key: "running", step: 3, title: "Observe ton robot", text: "Regarde son trajet et le bloc actif. Tu peux utiliser Arrêter à tout moment.", target: "[data-kidx-stop]" };
  ...
  if (state.error) return { key: "error", step: 3, title: "Vérifie le capteur", text: `${state.error} Ouvre Laboratoire + pour ajuster ton programme.`, target: state.labOpen ? "[data-code-editor]" : "[data-kidx-lab]" };
  ...
  if (mission.code || state.labOpen) {
    if (!state.labOpen) return { key: "lab", step: 1, title: "Découvre le programme", text: "Ouvre Laboratoire +, puis choisis Exemple de la mission. Il prépare les moteurs et les capteurs de ce défi.", target: "[data-kidx-lab]" };
