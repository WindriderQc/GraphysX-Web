 ...
 * the bad value was then silently dropped at render time. Parsing into two different
 * sentinels and comparing is the check that actually works: a string three understands
 * overwrites both, so they end up equal; one it does not leaves each at its own sentinel.
 * Same validator as agent-world-crowd.ts, where the failure was first measured.
 */
