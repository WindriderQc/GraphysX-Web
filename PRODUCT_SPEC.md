...
- **§4 scene model** — of the listed simulation systems, spline followers, particles/emitters,
  flocking and now **force fields** are real. ~~Crowds and evolutionary entities are not.~~
  **Both are now real too**: `dna-tree` (`dna-r2`) and `crowd` (`crowd-r1`), which completes
  the listed set. `crowd` graduates the population and its steering only — infection and
  squash stay game concerns for the rules vocabulary, and the members stopped being rigid
  bodies, so an explicit separation pass replaces the spacing the solver used to give free.
  That substitution is recorded as an adaptation in `agent-world-crowd.ts`, not as recovery.
  Force
...
- **§6 skyboxes** — ~~cube maps ship but are unreachable; `environment.background` is a single
