...
2026-09-14. **Public release `0a13e75af0882220a5716ec02c106eb720c9af31` is deployed.** [Run34873742082](https://github.com/WindriderQc/GraphysX-Web/actions/runs/34873742082) passed the full 84-journey inventory and the activated-site smoke. Public `release.json` was verified after completion; the normal browser shows the sculpted face and restores the saved conversation. The delivery checkout is `codex/llmx-public-release` at `C:\Users\Yanik\codes\GraphysX-Web-llmx-verified`; subsequent documentation commits do not change the published product SHA.

The common release combines Claude's sculpted lighting (`30bcb5a`) and MCP reconnection (`2a4f7be`), replacing the older MCP-only verification by agreement with that task. The first attempt passed 83 journeys; Family exceeded its ten-minute deadline without an assertion failure. One unchanged retry of its group passed, with Family in 8m18s; the other eleven groups were retained. No deadline, assertion or renderer quality was changed. The complete workflow took 53m02s including this retry; deployment took 4m42s. Receipts: `output/llmx-sculpted-face-release.json`, `output/llmx-sculpted-face-ci-coverage.json` and `output/llmx-sculpted-face-ci-final.log`.
...
- The combined sculpted-face/MCP release passes 563 Node tests with one existing Windows skip locally, 564 without skips on Linux CI, typecheck, lint, build, both Rapier probes and the local LLMx smoke (1m15s). All 84 registered journeys have passing coverage across the first attempt and its single group retry. The existing Family journey also passes locally in 6m12s with no browser errors; its mobile capture was inspected. The unchanged installed game skill client passes; its inspected ready-face capture has 23,091 cubes and an actual native opening/voice sample. Local receipts: `output/llmx-sculpted-face-merge-verify.log`, `output/playwright/llmx-sculpted-face-ready/` and `output/playwright/llmx-sculpted-face-family/`.
