...
| `src/llmx-face-finish.ts` | The forge finish on the mask's metal, patched into the standard material: a copper Fresnel rim; smooth normals, so each cube of the shell shades with the normal of the surface it belongs to (estimated in `llmx-face-pose.ts` by a plane fit over its neighbours) instead of its six faces, which is what removed the bright contour line along the cheek; and a camera-side fill that gives back the light the top faces used to catch. All three are uniforms, tunable live. |
