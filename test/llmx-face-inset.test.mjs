...
import { Group, PointLight, Scene, Vector2, Vector4 } from 'three';
...
function fixture(t) {
  ...
  const originalTarget = {}, scene = new Scene(), light = new PointLight();
  let effect = new Group(); scene.add(effect, light);
  const renderer = {
    ...
    render(renderedScene, camera) {
      ...
      assert.equal(effect?.visible ?? false, false, 'world thinking particles cannot cover the portrait');
      assert.equal(light.visible, true, 'the existing face lighting is retained');
  ...
  const inset = createLlmXFaceInset(renderer, scene, { append() {}, classList: { toggle() {}, remove() {} } }, () => face, () => {}, () => effect);
  t.after(() => inset.dispose());
  return { inset, renderer, button, view, originalTarget, effect, replace(value) { face = value; }, replaceEffect(value) { effect = value; } };
}

test('portrait maps CSS bounds into the shared renderer and restores every changed render state, including on failure', t => {
  const { inset, renderer, originalTarget, effect } = fixture(t);
  for (const fail of [false, true]) {
    ...
    assert.equal(effect.visible, true, 'the main world keeps its thinking effect even after a render failure');
  }
  assert.equal(inset.state().frames, 1);
});

test('portrait preserves a hidden effect and resolves newly spawned or removed thinking particles', t => {
  const { inset, renderer, effect, replaceEffect } = fixture(t);
  inset.update(true); effect.visible = false;
  inset.render(); assert.equal(effect.visible, false);
  const replacement = new Group(); replaceEffect(replacement);
  inset.render(); assert.equal(replacement.visible, true);
  replaceEffect(null); inset.render();
  assert.equal(renderer.calls.length, 3);
});
